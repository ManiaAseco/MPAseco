...
function chat_stats($aseco, $command) {
  ...
  $query = 'SELECT Hits, Deaths FROM  `players` WHERE Login =  ' . quotedString($target->login);
  $result = mysql_query($query);
  ...
  $stats[] = array('Kill/Death Ratio:', $k_d);
  if ($feature_ranks) {
