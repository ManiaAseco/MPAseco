...
class Aseco {
  ...
  function Aseco($debug) {
    ...
    $this->currstatus = 0;
    //Variables to avoid multiple callbacks
    $this->beginmapvar = false;   
    $this->endmapvar = false; 
    $this->beginroundvar = false;    
    $this->endroundvar = false;    
  }  // Aseco
...
/*      // set filename of banned IPs list file
      ...
      $style_file = 'configs/styles/' . $this->settings['window_style'] . '.xml';
      ...
      $panelbg_file = 'configs/panels/' . $this->settings['panel_bg'] . '.xml';
...
//    $this->server->packmask = $this->client->getResponse();
   ...
   */
  function beginRound($round_number = 0) {
    $this->endroundvar = false;
    if(!$this->beginroundvar){
      $this->console_text('Begin Round');
      $this->releaseEvent('onBeginRoundNr', $round_number); 
      $this->releaseEvent('onBeginRound', $this->smrankings);
      $this->beginroundvar = true;
    }
  }  // beginRound
   ...
   */
  function endRound($round_number = 0) {
    $this->beginroundvar = false;
    if(!$this->beginroundvar){
      $this->console_text('End Round');
      $this->releaseEvent('onEndRoundNr', $round_number); 
      $this->releaseEvent('onEndRound', $this->smrankings);
      $this->endroundvar = true;
    }
  ...
  function beginMap($race) {
    ...
    // ... and callback to function newMap()
      if(!$this->beginmapvar){
        $this->smrankings = array();
        $this->beginmapvar = true;
  ...
  function playerConnect($player) {
    ...
    $this->console('{1} connected', $login);
    if(strpos($login, "*fakeplayer") === true){   
      return;
    }
