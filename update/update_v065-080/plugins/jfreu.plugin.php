...
Aseco::registerEvent('onPlayerInfoChanged', 'kick_speconly');  
...
function kick_speconly($aseco, $playerInfo)
{
  $whi = $aseco->server->jfreu->white;
  $yel = $aseco->server->jfreu->yellow;
  $red = $aseco->server->jfreu->red;
  ...
  if($playerInfo["SpectatorStatus"] == 0){
        ...
        $aseco->server->jfreu->playerlist[$login]->speconly)
    {
      if ($player = $aseco->server->players->getPlayer($login))
      {
        $message = $yel.'>> '.$blu.'SpecOnly '.$whi.clean_nick($player->nickname).$blu.' tried to join the game ['.$red.'Kicked'.$blu.' ]';
        $aseco->client->query('ChatSendServerMessage', $message);
        // log console message
