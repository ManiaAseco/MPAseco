...
class ScriptSettings extends Plugin {
  ...
  function maniaLink(){
    ...
    $xml= '<frame pos="0.71 0.47 -100">
              ...
              <quad pos="-0.055 -0.045 -0.2" size="0.09 0.09" halign="center" valign="center" style="Icons128x128_1" substyle="NewTrack"/>
    ...
    foreach($scriptSettings as $key => $value) {
      ...
      $xml .= '<label pos="'.$px.' '.$py.' -0.6" size="0.75 0.06" halign="left" style="TextCardSmallScores2" text="'.$key.'"/>'; 
        
      if($substyle)
        $xml .= '<quad pos="'.($px-0.41).' '.$py.' -0.6" size="0.03 0.03" halign="center" style="Icons64x64_1" substyle="'.$substyle.'" action="'.$this->pluginMainId.$i.'"/>';   
      else
        $xml .= '<entry pos="'.($px-0.4).' '.$py.' -0.6" sizen="10 2" style="TextValueSmall" substyle="BgCard" name="'.$key.'" default="'.$value.'"/>';
